...
#import "XXCustomFlowLayout.h"


@interface XXCustomFlowLayout ()
...
@implementation XXCustomFlowLayout
...
- (UICollectionViewLayoutAttributes *)layoutAttributesForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    CGFloat interitemSpacing = CGFLOAT_MIN, lineSpacing = CGFLOAT_MIN;
    CGFloat x = 0, y = 0;
    ...
    if (indexPath.row == 0) {
        ...
        x = sectionInsets.left + (itemSize.width + interitemSpacing) * ([minColumn intValue] - 0);
        y = [self.maxYDic[minColumn] floatValue];
...
- (BOOL)shouldInvalidateLayoutForBoundsChange:(CGRect)newBounds {
    //  宽度发生变化,一般不会
    if (CGRectGetWidth(newBounds) != CGRectGetWidth(self.collectionView.bounds)) return YES;
    return NO;
}
...
#pragma mark - Getter
- (CGFloat)currentMaxY {
    if (!self.maxYDics.lastObject) return 0;
